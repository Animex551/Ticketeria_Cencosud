...
function Ticketeria() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const esVistaTomados = location.pathname.includes("TomadosTPage");
        
        // URL base (Mis Tickets)
        let url = `http://localhost:8000/api/tickets/?cliente=${emailLogueado}`;

        if (esVistaTecnico) {
          // Tickets disponibles (Abiertos)
          url = `http://localhost:8000/api/tickets/?vista=abiertos&cliente=${emailLogueado}`;
        } else if (esVistaTomados) {
          // Tickets tomados por el técnico
          url = `http://localhost:8000/api/tickets/?vista=tomados&tecnico=${emailLogueado}`;
        }
  ...
  // Función para definir el título dinámico
  const obtenerTitulo = () => {
    if (location.pathname.includes("TecnicoPage")) return "Tickets Abiertos de otros usuarios";
    if (location.pathname.includes("TomadosTPage")) return "Tickets Tomados (En Proceso)";
    return "Mis Tickets";
  };
